...
namespace SampleCourier.TrackingService.Service
{
    ...
    public class RoutingSlipStateMachine : MassTransitStateMachine<RoutingSlipState>
    {
        ...
        private static void HandleRoutingSlipCreated(BehaviorContext<RoutingSlipState, RoutingSlipCreated> context) =>
            context.Instance.CreateTime = context.Data.Timestamp;

        private static void HandleRoutingSlipCompleted(BehaviorContext<RoutingSlipState, RoutingSlipCompleted> context)
        {
            context.Instance.EndTime = context.Data.Timestamp;
            context.Instance.Duration = context.Data.Duration;
        }

        private static void HandleRoutingSlipFaulted(BehaviorContext<RoutingSlipState, RoutingSlipFaulted> context)
        {
            context.Instance.EndTime = context.Data.Timestamp;
            context.Instance.Duration = context.Data.Duration;
        ...
        private Task LogAsync(Guid trackNum, string msg, string state)
        {
